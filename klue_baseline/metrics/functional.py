...
def klue_ner_entity_macro_f1(preds: np.ndarray, labels: np.ndarray, label_list: List[str]) -> Any:
    ...
    entity_macro_f1 = ner_f1_score([labels_label], [preds_label], average="macro", mode="strict", scheme=IOB2) #macro
...
def klue_ner_char_macro_f1(preds: np.ndarray, labels: np.ndarray, label_list: List[str]) -> Any:
    ...
    return sklearn.metrics.f1_score(trues, preds, labels=label_indices, average="macro", zero_division=True) * 100.0 #macro


def klue_re_micro_f1(preds: np.ndarray, labels: np.ndarray) -> Any:
    """KLUE-RE micro f1 (except no_relation)"""
    return sklearn.metrics.f1_score(labels, preds, average="macro", labels=list(range(1, 30))) * 100.0
